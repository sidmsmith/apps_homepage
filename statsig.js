...
function getAppInfo() {
  ...
  // Extract version from title like "SC Utilities v1.2.3"
  const versionMatch = title.match(/v(\d+\.\d+\.\d+)/i);
  const version = versionMatch ? versionMatch[1] : '1.2.3'; // Default from title
  // Extract app name (everything before "v" or just use title)
  const appNameMatch = title.match(/^(.+?)\s+v\d+/i);
  const appName = appNameMatch ? appNameMatch[1].trim() : (title.split(' v')[0] || 'SC Utilities');
